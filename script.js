...
function mensagensChegaram(resposta){
    ...
    for(let i=0;i<todasMensagens.length;i++){
        if(todasMensagens[i].text === 'entra na sala...' || todasMensagens[i].text === 'sai da sala...'){
            corpoDasMensagens.innerHTML = corpoDasMensagens.innerHTML + `<div class ='entrada-saida'><p><span class='horario'>(${todasMensagens[i].time})</span><span class='usuario'>  ${todasMensagens[i].from}</span> para <span class='usuario'>${todasMensagens[i].to}</span>:  <span>${todasMensagens[i].text}</span></p></div>`;            
        }else if(todasMensagens[i].to === 'Todos'){
            corpoDasMensagens.innerHTML = corpoDasMensagens.innerHTML + `<div class ='mensagem-todos'><p><span class='horario'>(${todasMensagens[i].time})</span><span class='usuario'>  ${todasMensagens[i].from}</span> para <span class='usuario'>${todasMensagens[i].to}</span>:  <span>${todasMensagens[i].text}</span></p></div>`;
        }else{
            corpoDasMensagens.innerHTML = corpoDasMensagens.innerHTML + `<div class ='mensagem-reservada'><p><span class='horario'>(${todasMensagens[i].time})</span><span class='usuario'>  ${todasMensagens[i].from}</span> reservadamente para <span class='usuario'>${todasMensagens[i].to}</span>:  <span>${todasMensagens[i].text}</span></p></div>`;
        }
